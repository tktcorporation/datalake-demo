...
class PleaseJS {
    ...
    static random_int(min, max, randomiser) {
        ...
        if (randomiser instanceof PleaseJS.RC4Random) {
            random = randomiser.random;
        }
        return Math.floor(random() * (max - min + 1)) + min;
    }

    static random_float(min, max, randomiser) {
        var random = Math.random;
        if (randomiser instanceof PleaseJS.RC4Random) {
            random = randomiser.random;
        }
    ...
    static convert_to_format(format_string, array) {
        var i;
        switch (format_string) {
            case 'hex':
                for (i = 0; i < array.length; i++) {
                    array[i] = PleaseJS.HSV_to_HEX(array[i]);
                }
                break;
            case 'rgb':
                for (i = 0; i < array.length; i++) {
                    array[i] = PleaseJS.HSV_to_RGB(array[i]);
                }
                break;
            case 'rgb-string':
                for (i = 0; i < array.length; i++) {
    ...
    static NAME_to_HEX(name) {
        name = name.toLowerCase();
        if (name in PleaseJS.color_data) {
            return PleaseJS.color_data[name];
        } else {
    ...
    static make_color(options) {
        ...
        if (typeof color_options.seed === 'string') {
            randomiser = new PleaseJS.RC4Random(color_options.seed);
        }
        ...
        for (var i = 0; i < color_options.colors_returned; i++) {
            ...
            if (base_color !== null) {
                ...
                if (base_color.s === 0) {
                    ...
                    saturation = PleaseJS.random_float(0.4, 0.85, randomiser);
                }
                value = PleaseJS.random_float(0.4, 0.85, randomiser);
                color.push({
                    h: hue,
                    s: saturation,
                    v: value
                ...
                if (color_options.greyscale === true || color_options.grayscale === true) {
                    ...
                    saturation = PleaseJS.clamp(color_options.saturation, 0, 1);
                }
                //set value
                if (color_options.full_random === true) {
                    ...
                    value = PleaseJS.clamp(color_options.value, 0, 1);
                }
                color.push({
                    h: hue,
                    s: saturation,
    ...
    static make_contrast(HSV, options) {
        ...
        var contrast,
            value_range = PleaseJS.contrast_quotient(HSV),
            adjusted_hue;
